...
`--analyze` принадлежит только `decode` и сохранён для совместимости: полный анализ кодов уже включён по умолчанию, дополнительная повторная сводка не печатается. `auto` уже анализирует коды без этого флага. `decode` выводит единый анализ EN/RU, включая российские сокращения; `auto`/`batch` сохраняют оба анализа в TXT. [Python API](SUPPORTED_CODES.md) позволяет ограничить категории профилем службы.
